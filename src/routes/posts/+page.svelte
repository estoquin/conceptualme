    	...
    	<section class="mb-2">
    		<div class="flex flex-col md:flex-row md:items-center md:justify-between border-b pb-4">
    			...
    			<div class="flex gap-2 mt-2 md:mt-0 md:ml-4">
					<button class="px-2 py-1 bg-green-600 text-white text-sm font-semibold hover:bg-green-800 transition" style="border-radius:0;" onclick={() => listMode = false} title="Card View">&#9632;</button>
					<button class="px-2 py-1 bg-green-600 text-white text-sm font-semibold hover:bg-green-800 transition" style="border-radius:0;" onclick={() => listMode = true} title="List View">&#9776;</button>
    			</div>
    		</div>
    	</section>
    	<section class={listMode ? "flex flex-col gap-4" : "grid md:grid-cols-2 lg:grid-cols-3 gap-6"}>
    		{#each data.posts as post}
    			<PostCard {post} headingLevel="h2" {listMode} colorFilter="green">
